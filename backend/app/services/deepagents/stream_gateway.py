...
from app.services.deepagents.runtime_resolver import resolve_deepagents_runtime
...
def parse_stream_request(raw_payload: Any) -> ParsedDeepAgentsStreamRequest:
...
def serialize_stream_part(chunk: Any) -> tuple[str, Any, list[str]] | None:
    ...
    if event_type == "messages":
        ...
        metadata_record = metadata_payload if isinstance(metadata_payload, dict) else {}
        if ns:
        ...
        return (event_type, [message_payload, metadata_record], ns)
...
def serialize_agent_topology(agent: Any) -> dict[str, Any] | None:
...
def build_deepagents_stream_response(
    payload: dict[str, Any],
    *,
    current_user: User,
) -> StreamingResponse:
    def _event_iter():
        stream_request = parse_stream_request(payload)
        req_id = uuid4().hex[:10]
        started = _now_ms()
        heartbeat_count = 0
        ...
        def _push(event: str, data: Any = None, *, ns: list[str] | None = None) -> None:
            ...
            event_payload = _json_safe(data) if data is not None else {}
            _LOG.debug(
                ...
                event_name,
                ",".join(
                    sorted([str(k) for k in list(event_payload.keys())[:8] if str(k)])
        ...
        def _worker() -> None:
            ...
            try:
                ...
                agent, _, _, files_mapping = build_chat_agent(
                ...
                topology = serialize_agent_topology(agent)
                ...
                for chunk in agent.stream(
                    ...
                    serialized = serialize_stream_part(chunk)
                ...
                _push("error", {"message": str(exc)[:500] or "deepagents stream error"})
            finally:
                worker_db.close()
