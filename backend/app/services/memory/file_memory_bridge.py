...
class FileMemoryBridge:
    ...
    def _cache_ttl_seconds(self) -> float:
        try:
            return max(0.0, float(getattr(settings, "aelin_base_context_cache_ttl_seconds", 4.0) or 0.0))
        except Exception:
            return 4.0

    def _cache_max_entries(self) -> int:
        try:
            return max(8, int(getattr(settings, "aelin_base_context_cache_max_entries", 128) or 128))
        except Exception:
            return 128
