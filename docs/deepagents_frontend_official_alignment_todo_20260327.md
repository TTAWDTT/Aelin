...
- [ ] 让 `chatStore` 只保留 UI 偏好、本地草稿、当前 session/workspace、pane 开关，而不再保存 canonical message list。
- [ ] 删除 `stream -> chatStore -> timeline` 的双轨同步路径。
...
- [ ] 让消息气泡优先直接使用 `stream.getToolCalls(message)` 渲染工具调用。
