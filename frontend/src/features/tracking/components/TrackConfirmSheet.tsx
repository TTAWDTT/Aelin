  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={onClose}>
      ...
      <div onClick={e => e.stopPropagation()}
        className="relative w-full sm:max-w-md bg-[var(--color-panel)] border border-[var(--color-border)] rounded-t-2xl sm:rounded-2xl p-5 space-y-4">
        ...
        <label className="block text-xs space-y-1">
          ...
          <input value={form.target} onChange={e => setForm(p => ({...p, target: e.target.value}))} placeholder="例：某个人、某个关键词、某个网站URL…" className="aelin-input" />
        </label>

        <label className="block text-xs space-y-1">
          <span className="text-[var(--color-text-muted)]">数据来源</span>
          <select value={form.source} onChange={e => setForm(p => ({...p, source: e.target.value}))} className="aelin-select">
        ...
        <label className="block text-xs space-y-1">
          ...
          <select value={form.interval} onChange={e => setForm(p => ({...p, interval: e.target.value}))} className="aelin-select">
        ...
        <label className="block text-xs space-y-1">
          ...
          <input value={form.description} onChange={e => setForm(p => ({...p, description: e.target.value}))} placeholder="补充说明" className="aelin-input" />
        </label>

        <button onClick={() => confirm.mutate()} disabled={!form.target.trim() || confirm.isPending}
          className="aelin-btn aelin-btn-primary w-full">
